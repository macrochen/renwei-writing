...
6. **强制输出改动对照与理由（最核心步骤）。** 任何一次修改，都必须明确、逐处地交代：**具体修了哪些地方（原句 vs 改后）**，以及**为什么要这样改（基于人味儿原则的理由）**。哪怕只删改了一个字或标点，也要清晰列出。拿不准的改动明确说"这处拿不准,可以还原"。把否决权留给作者。
...
- **内容结构**：生成的文件必须包含两个明确的独立章节：
  1. **【打磨后的最终文本】**
  2. **【具体修改对照与理由】**：以列表形式逐一列出“原文”、“改后”，并给出详细且具有说服力的“修改理由”（必须紧扣“人味儿”原则解释）。
- **交互要求**：在回复用户时，必须给出文件的绝对路径，并用 1-2 句话点拨核心改动点，无需在对话框里打印完整的长篇内容。
