...
6. **强制输出改动对照与理由（最核心步骤）。** 任何一次修改，都必须明确、逐处地交代：**具体修了哪些地方（使用 diff 格式对比原句与改后）**，以及**为什么要这样改（基于人味儿原则的理由）**。哪怕只删改了一个字或标点，也要清晰列出。拿不准的改动明确说"这处拿不准,可以还原"。把否决权留给作者。
7. **上限是隐形。** 你帮人改文字的最好结果,是读者完全感觉不到
   你来过。不要追求"改得更好",追求"清掉绊脚的,其他原样"。

## 输出规范

执行完毕后，必须将最终打磨后的文本与逐处改动说明，分别保存为两个独立的 Markdown 文件。由于产生多个文件，应建立专属的子目录：
- **保存目录**：`~/outputs/renwei-writing-skill/<yyyy-mm-dd-简短描述>/` (例：`~/outputs/renwei-writing-skill/2026-06-14-文案打磨/`)
- **生成文件**：
  1. `final.md`：仅包含打磨后的最终文本。
  2. `changelog.md`：【具体修改对照与理由】，必须使用 Markdown 代码块包裹的 diff 格式（以 `-` 和 `+` 标识删减和增加的内容）直观展示对比，并在每一处修改下方给出详细且具有说服力的“修改理由”（必须紧扣“人味儿”原则解释）。
- **交互要求**：在回复用户时，必须分别给出这两个文件的绝对路径，并用 1-2 句话点拨核心改动点，无需在对话框里打印完整的长篇内容。
